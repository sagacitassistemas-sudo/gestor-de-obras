...
export const CadastroEtapaModal: React.FC<CadastroEtapaModalProps> = ({
  ...
  const [dataInicioFinanceiro, setDataInicioFinanceiro] = useState<string>("");
  const [dataFimFinanceiro, setDataFimFinanceiro] = useState<string>("");
  ...
  const parentOptions = useMemo(() => {
    return existingItems
      .filter((i) => {
        if (
          itemToEdit &&
          (i.eap_codigo === itemToEdit.eap_codigo ||
            i.eap_codigo.startsWith(itemToEdit.eap_codigo + "."))
        )
          return false;
        return !i.e_analitico && i.eap_codigo.split(".").length < 4;
      })
      .sort((a, b) =>
        a.eap_codigo.localeCompare(b.eap_codigo, undefined, { numeric: true }),
      );
  ...
  const calculateSuggestedEap = (pCode: string): string => {
    ...
    const children = existingItems.filter(
      (i) =>
        i.eap_codigo.startsWith(prefix) &&
        i.eap_codigo.split(".").length === pCode.split(".").length + 1,
    );
    const nums = children
      .map((i) => Number(i.eap_codigo.split(".").pop()))
      .filter((n) => !isNaN(n));
  ...
  useEffect(() => {
    ...
    if (itemToEdit) {
      ...
      setQuantidadeContratada(String(itemToEdit.quantidade_contratada ?? 1));
      setPrecoUnitario(
        String(
          itemToEdit.preco_unitario ?? itemToEdit.valor_total_contratado ?? 0,
        ),
      );
      setDataInicioFinanceiro(itemToEdit.data_inicio_financeiro || "");
      setDataFimFinanceiro(itemToEdit.data_fim_financeiro || "");
      // Parsear predecessores existentes
      setPredEntries((itemToEdit.predecessores ?? []).map(parsePredStr));
    } else {
      setParentCode("root");
      ...
      setDataExecucao(
        projetoDataInicio || new Date().toISOString().split("T")[0],
      );
  ...
  const handleParentChange = (newPCode: string) => {
    ...
    if (!itemToEdit && !isManualEap) {
      ...
      if (newPCode !== "root" && newPCode.split(".").length >= 2)
        setEAnalitico(true);
  ...
  const handleQtdPriceChange = (qtdStr: string, priceStr: string) => {
    ...
    setValorTotalContratado(
      ((parseFloat(qtdStr) || 0) * (parseFloat(priceStr) || 0)).toFixed(2),
    );
  };

  // ── Gestão de predecessoras ────────────────────────────────────────────────

  const isPredSelected = (code: string) =>
    predEntries.some((p) => p.code === code);
  ...
  const updatePredType = (code: string, type: PredEntry["type"]) => {
    setPredEntries((prev) =>
      prev.map((p) => (p.code === code ? { ...p, type } : p)),
    );
  };

  const updatePredLag = (code: string, lag: number) => {
    setPredEntries((prev) =>
      prev.map((p) => (p.code === code ? { ...p, lag } : p)),
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!eapCodigo.trim() || !descricaoServico.trim()) {
      setErrorMessage(
        "Código EAP e Descrição/Nome do Serviço são obrigatórios.",
      );
      return;
    }
    if (eAnalitico && !unidadeMedida.trim()) {
      setErrorMessage(
        "Unidade de medida é obrigatória para serviços executáveis/analíticos.",
      );
      return;
    }
    ...
    try {
      ...
      const payload: any = {
        ...
        quantidade_contratada: eAnalitico
          ? parseFloat(quantidadeContratada) || 1
          : 0,
        valor_total_contratado: eAnalitico
          ? parseFloat(valorTotalContratado) || 0
          : 0,
        ...
        data_inicio_financeiro: dataInicioFinanceiro || null,
        data_fim_financeiro: dataFimFinanceiro || null,
      };
      ...
      if (res.ok && (json.success || json.id || json.item_eap_id)) {
        ...
        console.warn(
          "[CadastroEtapaModal] Fallback para Supabase direto:",
        ...
        const upsertData: any = {
          ...
          valor_total_contratado: eAnalitico
            ? parseFloat(valorTotalContratado) || 0
            : 0,
          valor_desembolsado: eAnalitico
            ? parseFloat(valorDesembolsado) || 0
            : 0,
          e_analitico: eAnalitico,
          duracao_dias: Math.max(1, Number(duracaoDias || 1)),
          predecessores,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold tracking-tight">
                {itemToEdit
                  ? `Editar Etapa: ${itemToEdit.eap_codigo}`
                  : "Cadastrar Nova Etapa (EAP)"}
              </h3>
        ...
        <form
          ...
          className="p-6 overflow-y-auto space-y-5 flex-1 text-slate-800"
        >
          {errorMessage && (
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
              <div>
                <span className="block text-sm font-bold text-slate-900">
                  É um Serviço Executável (Analítico)?
                </span>
                <span className="text-xs text-slate-500">
                  {eAnalitico
                    ? "Etapa executável (Nível 3) com duração, quantidades e valores."
                    : "Agrupador de etapas (Níveis 1 ou 2)."}
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={duracaoDias}
                  onChange={(e) =>
                    setDuracaoDias(parseInt(e.target.value) || 1)
                  }
                  disabled={!eAnalitico}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {predEntries.length > 0 && (
                  <span className="text-[11px] bg-[#005daa] text-white px-2 py-0.5 rounded-full font-bold">
                    {predEntries.length} definida
                    {predEntries.length > 1 ? "s" : ""}
                  </span>
                )}
              </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="truncate max-w-[220px] text-slate-600">
                              {p.descricao_servico}
                            </span>
                          </div>
                          {p.duracao_dias && (
                    ...
                    <div className="space-y-2 pt-1">
                      ...
                      {predEntries.map((entry) => {
                        const opt = predecessorOptions.find(
                          (p) => p.eap_codigo === entry.code,
                        );
                        return (
                          <div
                            key={entry.code}
                            className="bg-white rounded-lg border border-slate-200 p-3 flex flex-wrap items-center gap-3"
                          >
                            {/* Badge do tipo atual */}
                            <span
                              className={`text-[11px] px-2 py-0.5 rounded border font-bold font-mono ${DEP_COLORS[entry.type]}`}
                            >
                              {entry.type}
                            </span>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  updatePredType(
                                    entry.code,
                                    e.target.value as PredEntry["type"],
                                  )
                                }
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {Object.entries(DEP_LABELS).map(([k, label]) => (
                          <span
                            key={k}
                            className={`text-[10px] px-2 py-0.5 rounded border font-medium ${DEP_COLORS[k]}`}
                          >
                            {label}
                          </span>
                        ))}
          ...
          {eAnalitico && (
            <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-xl space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleQtdPriceChange(e.target.value, precoUnitario)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    value={precoUnitario}
                    onChange={(e) =>
                      handleQtdPriceChange(quantidadeContratada, e.target.value)
                    }
                    className="w-full bg-white border border-slate-300 font-mono font-bold text-sm text-slate-900 rounded-lg px-3.5 py-2.5 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 outline-none transition-all"
                  />
                </div>
              ...
              <div className="pt-2 border-t border-emerald-200">
                <h5 className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-3">
                  Datas Específicas de Desembolso (Opcional)
                </h5>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-300 text-sm font-medium text-slate-900 rounded-lg px-3.5 py-2.5 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 outline-none transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                ...
                <p className="text-[10px] text-slate-500 mt-2 font-medium">
                  Se não preenchidas, as datas do Cronograma Físico-Financeiro
                  serão sincronizadas automaticamente com as datas do Cronograma
                  Executivo (Gantt).
                </p>
              </div>
            </div>
